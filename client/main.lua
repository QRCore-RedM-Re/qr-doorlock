			...
			if distance < maxDistance then
				if distance < 1.75 then
					if IsControlJustPressed(0,Config.KeyPress) and CoolDown < 1 then
						...
						TriggerServerEvent("qr-doorlock:updatedoorsv", k, state)
					end
				end
			end
...
RegisterNetEvent('qr-doorlock:changedoor')
AddEventHandler('qr-doorlock:changedoor', function(doorID, state)
	...
	if not IsEntityPlayingAnim(ped, "script_common@jail_cell@unlock@key", "action", 3) then
		...
		AttachEntityToEntity(prop, ped,boneIndex, 0.02, 0.0120, -0.00850, 0.024, -160.0, 200.0, true, true, false, true, 1, true)
			Wait(250)
		TriggerServerEvent('qr-doorlock:updateState', doorID, state, function(cb) end)
			Wait(1500)
...
RegisterNetEvent('qr-doorlock:setState')
AddEventHandler('qr-doorlock:setState', function(doorID, state)
